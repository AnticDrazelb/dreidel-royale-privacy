...
package's **real source**, at the versions `Packages/manifest.json` pins:

```sh
cd unity/tools/apicheck
python3 verify.py --fetch     # clone the pinned package sources
python3 verify.py             # 63 claims, non-zero exit if any is wrong
```

This exists because the stubs below cannot catch the mistake that matters most. They are
written from the same understanding as the code, so when that understanding is wrong about
a package API, the stub is wrong in the same direction and agrees. Four builds in one
evening failed that way — a renamed type, a moved namespace, an extension method mistaken
for a member, and a method that never existed. The real source disagrees; a stub does not.
See `tools/apicheck/README.md`.
